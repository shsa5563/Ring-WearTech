...
void htmTemperatureMeasure(void)
{
  ...
  //gecko_cmd_hardware_set_soft_timer(TIMER_MS_2_TIMERTICK(htmTempMeas.period), TEMP_TIMER, true); // shekhar : comment this
}
...
static uint8_t htmBuildTempMeas(uint8_t *pBuf, htmTempMeas_t *pTempMeas)
{
  ...
  UINT16_TO_BITSTREAM(p, pTempMeas->xValue);
  UINT16_TO_BITSTREAM(p, pTempMeas->yValue);
  UINT16_TO_BITSTREAM(p, pTempMeas->zValue);
...
static uint8_t htmProcMsg(uint8_t *buf)
{
  ...
  htmTempMeas.xValue = xtempValue;
  htmTempMeas.yValue = ytempValue;
  htmTempMeas.zValue = ztempValue;
  htmTempMeas.temperature= bma280_temp;
