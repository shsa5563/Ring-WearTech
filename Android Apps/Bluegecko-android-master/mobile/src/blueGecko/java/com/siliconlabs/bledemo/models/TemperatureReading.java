...
public class TemperatureReading {
    ...
    public TemperatureReading(Type type, double temperature, long readingTime, HtmType htmType,short xValue, short yValue, short zValue, int tapState) {
        this.type = type;
    ...
    public static TemperatureReading fromCharacteristic(BluetoothGattCharacteristic characteristic) {
        ...
        int  _xValue =  characteristic.getIntValue(BluetoothGattCharacteristic.FORMAT_UINT16, 12);
        int  _yValue = characteristic.getIntValue(BluetoothGattCharacteristic.FORMAT_UINT16, 14);
        int  _zValue = characteristic.getIntValue(BluetoothGattCharacteristic.FORMAT_UINT16, 16);

        short xValue = (short)_xValue;
        short yValue = (short)_yValue;
        short zValue = (short)_zValue;

        int  tapState = characteristic.getIntValue(BluetoothGattCharacteristic.FORMAT_UINT8, 18);

        HtmType readHtmType = HtmType.UNKNOWN;
        if((flags & 0x04) > 0) {
            readHtmType = HtmType.values()[characteristic.getIntValue(BluetoothGattCharacteristic.FORMAT_UINT8, 19)];
        }

        return new TemperatureReading(type, actualTemp, time, readHtmType, xValue, yValue, zValue, tapState);
    }
    ...
    public int getTapState() {return tapState;}
    ...
    public static TemperatureReading getSampleReading() {
        ...
        return new TemperatureReading(Type.FAHRENHEIT, temp, time, HtmType.UNKNOWN, (short)3, (short)4,(short)5, (int)0);
    }
