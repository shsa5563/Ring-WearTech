...
public class MainActivity extends AppCompatActivity {
    ...
    public void handleSendText(Intent intent) {
        ...
        if (sharedText != null) {
            ...
            /* String xVal = sharedText.substring(sharedText.lastIndexOf("X:") + 1, sharedText.lastIndexOf("Y:"));
            String yVal = sharedText.substring(sharedText.lastIndexOf("Y:") + 1, sharedText.lastIndexOf("Z:"));
            String zVal = sharedText.substring(sharedText.lastIndexOf("Z:") + 1, sharedText.lastIndexOf(":)"));
            Toast.makeText(MainActivity.this, "(s:" + xVal + ":" + yVal + ":" + zVal + ":"+")", Toast.LENGTH_LONG).show();*/
            // Update UI to reflect text being shared

            String[] AxValues = sharedText.split(",");
            Short [] AxShortVal= {0};

           /* for (int i=0; i < AxValues.length; i++) {
                System.out.println("Str["+i+"]:"+AxValues[i]);
            }

            for(int i=0; i<3 ;i++) {
                AxShortVal[i] = Short.parseShort(AxValues[i]);
            }*/
            short[] ints = new short[AxValues.length];
            for (int i = 0; i < AxValues.length; i++) {
                if(AxValues[i].contains("-")) {
                    AxValues[i].replace("-", "");
                    ints[i] = Short.parseShort(AxValues[i]);
                   // if (ints[i] < 0) {
                        values[i] = ((float) ints[i] / Math.abs(Short.MIN_VALUE)) ;
                 } else /*if (ints[i] > 0)*/ {
                        ints[i] = Short.parseShort(AxValues[i]);
                        values[i] = (float) ints[i] / Short.MAX_VALUE;
                }
            }


            this.getAccChartManager().onUpdateChart(values);
            // Are we Responsive?
            if(MainActivity.this.isResponsive()) {
                ...
                switch(MainActivity.this.getMode()) {
                    ...
                    case TRAINING    : {
                        ...
                        this.getTrainChartManager().onUpdateChart(values);
                    } break;
                    /** Are we recognizing? */
                    case RECOGNITION : {
                        // Update the Training Chart.
                        this.getRecognitionChartManager().onUpdateChart(values);
                    } break;
                }
            }


            Toast.makeText(MainActivity.this, "(s:" +sharedText+ ":"+ String.valueOf(values[0]) + String.valueOf(values[1])+String.valueOf(values[2])+")", Toast.LENGTH_LONG).show();
        }
    }
                        ...
                        case RECOGNITION  : {
                            ...
                            final AsyncTask lAsyncTask = new AsyncTask() { @Override protected final Object doInBackground(final Object ... pObjects) {
                                ...
                                MainActivity.this.runOnUiThread(new Runnable() { @Override public final void run() {
                                    ...
                                    // Print the Result.
                                    if( ((lAverages[0] < 0.1f )&&(lAverages[1] < 0.1f )) || ((lAverages[1] < 0.1f )&&(lAverages[2] < 0.1f )) ||((lAverages[2] < 0.1f )&&(lAverages[0] < 0.1f )) ) {
                                        Toast.makeText(MainActivity.this, "D(X:" + lAverages[0] + ", Y:" + lAverages[1] + ", Z:" + lAverages[2] + ": A match)", Toast.LENGTH_LONG).show();
                                    }
                                } });
